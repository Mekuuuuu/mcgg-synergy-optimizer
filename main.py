...
from helper import get_glory_league_hid, initialize_traits_and_heroes, find_best_team
...
locked_board = [
    hero_index.index(next(h for h in hero_index if h.name == "Benedetta")),
    hero_index.index(next(h for h in hero_index if h.name == "Benedetta")),
]
core_heroes = [
...
results = find_best_team(5, hero_index, traits_index, locked_heroes=locked_board, glory_league_ids=glory_league_ids, top_k=5)
